 ...
 *
 * @param shards A collection of individual [PlainResourceIndex] to be aggregated.
 */
class AggregatedResourceIndex(
    private val shards: Collection<PlainResourceIndex>
) : ResourcesIndex {
    ...
    private fun <R> tryShards(f: (shard: PlainResourceIndex) -> R?): R {
        shards.iterator()
            .forEach { shard ->
                val result = f(shard)
                if (result != null) {
