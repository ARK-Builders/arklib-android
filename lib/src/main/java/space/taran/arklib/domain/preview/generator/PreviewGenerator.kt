...
abstract class PreviewGenerator {
    ...
    companion object {
        ...
        fun storePreview(path: Path, bitmap: Bitmap) =
            storeImage(path, bitmap)

        fun storeThumbnail(path: Path, bitmap: Bitmap) {
        ...
        fun resizePreviewToThumbnail(preview: Bitmap): Bitmap =
            resizeToThumbnail(bitmapBuilder().load(preview))

        fun resizePreviewToThumbnail(path: Path): Bitmap =
